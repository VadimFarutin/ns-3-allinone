...
private:
  ...
  /** Flag is true if NetAnim will use online mode. */
  bool m_onlineMode;
  netanim::NetAnim * m_netAnim;
  QApplication * app;
  /** Parser from AnimatorMode. */
  ...
  // ##### General #####
  // Initialize NetAnim and QApplication
  ...
  void ParseAnim (bool routing = false);
  void ParseUpdateNodePosition (uint32_t nodeId, double x, double y);
  void ParseUpdateNodeColor (uint32_t nodeId, uint8_t r, uint8_t g, uint8_t b);
  void ParseUpdateNodeDescription (uint32_t nodeId);
  void ParseUpdateNodeSize (uint32_t nodeId, double width, double height);
  void ParseAddResource (uint32_t resourceId, std::string resourcePath);
  void ParseAddNodeCounter (uint32_t counterId, std::string counterName, CounterType counterType);
  void ParseUpdateNodeImage (uint32_t nodeId, uint32_t resourceId);
  void ParseUpdateNodeCounter (uint32_t counterId, uint32_t nodeId, double value);
  void ParseNode (uint32_t id, uint32_t sysId, double locX, double locY);
  void ParseLink (uint32_t fromId, uint32_t toLp, uint32_t toId);
  void ParseUpdateLink (uint32_t fromId, uint32_t toId, std::string);
  void ParseP (std::string pktType, 
                                 uint32_t fId, 
                                 double fbTx, 
                                 double lbTx, 
                                 uint32_t tId, 
                                 ...
                                 double lbRx,
                                 std::string metaInfo = ""); 
  void ParseP (uint64_t animUid, std::string pktType, uint32_t fId, double fbTx, double lbTx);
  void ParsePRef (uint64_t animUid, uint32_t fId, double fbTx, std::string metaInfo = "");
  void ParseClose (std::string name, bool routing = false);
  void ParseNonP2pLinkProperties (uint32_t id, std::string ipAddress, std::string channelType);
  void ParseRouting (uint32_t id, std::string routingInfo);
  void ParseRp (uint32_t nodeId, std::string destination, Ipv4RoutePathElements rpElements);
  void ParseUpdateBackground (std::string fileName, double x, double y, double scaleX, double scaleY, double opacity);
  void ParseIpv4Addresses (uint32_t nodeId, std::vector<std::string> ipv4Addresses);
  void ParseIpv6Addresses (uint32_t nodeId, std::vector<std::string> ipv4Addresses);
};
